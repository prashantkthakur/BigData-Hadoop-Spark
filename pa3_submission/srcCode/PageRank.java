...
//        top10.show();
        ...
        top10.createOrReplaceTempView("finaltbl");
        System.out.println("Final");
        top10.show();
        Dataset output = spark.sql("SELECT finaltbl.id, finaltbl.title, ranktbl.rank from finaltbl INNER JOIN ranktbl ON ranktbl.id=finaltbl.id");
        output.createOrReplaceTempView("outtbl");
        spark.sql("SELECT id, title, rank FROM outtbl ORDER BY rank DESC LIMIT 10").show();
        spark.stop();
